...
void set_intr_desc(unsigned char intr_no, void *handler) {
  idt[intr_no].offset_00_15 = (unsigned long long)handler & 0xffff;
  idt[intr_no].segment_selector = SS_KERNEL_CODE;
  idt[intr_no].type = DESC_TYPE_INTR;
  idt[intr_no].p = 1;
  idt[intr_no].offset_31_16 = ((unsigned long long)handler >> 16) & 0xffff;
  idt[intr_no].offset_63_32 = ((unsigned long long)handler >> 32) & 0xffffffff;
}

void intr_init(void) {
  void *addr;
  asm volatile("lea default_handler, %[addr]" : [ addr ] "=r"(addr));
  int i;
